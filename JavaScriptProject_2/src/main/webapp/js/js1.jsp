                     ...
                     ========        ===  ===
                                          ...
                                          2) 이항연산자
                                             ...
                                             비교연산자 : === (같다) , !==(같지 않다)
                                                      ...
                                                      <= (작거나 같다) , >= (크거나 같다)
                                                      => 결과값은 true/false
                                                      => 숫자외에 문자열, 날짜를 비교할 수 있다
                                             논리연산자 : && , ||
                                                          ====
                                                          둘 중에 한 개 이상만 true일때 true
                                                     ==== 두개의 조건이 true일 때 true 나머지는 false
                                             대입연산자 : = , += , -=
                                                          == HTML 결합 
                                          3) 삼항연산자 : (조건)?값1:값2
                                                      true => 값1
                              ...
                              ## 오라클 : 공유하는 데이터를 저장하는 공간
                                       ==============      
                              오라클 => 자바 => 자바스크립트로 전송 => HTML을 생성, CSS를 생성 => 화면에 출력              
                     1) 변수 : 한개의 데이터 저장 
                        => 자동 지정 변수 : var , let , const
                                       ===   ===   ===== 상수형 (변경하지 않는 값)
                                                         => 자바에서 데이터를 전송하면 받는 변수
                                             사용범위가 명확하다 => ES6
                                       | 사용범위가 명확하지 않다 => 메모리 해제가 안된다 ES5
                           function display()
                           {
                               var i=10;
                           }
